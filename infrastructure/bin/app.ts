...
const env = {
    account: process.env.CDK_DEFAULT_ACCOUNT || '655765967000',
    region: process.env.CDK_DEFAULT_REGION || 'us-east-1',
};

// =============================================
// STACK ÚNICO: ChatbotInteligenteStack
// =============================================
class ChatbotInteligenteStack extends cdk.Stack {
    constructor(scope: cdk.App, id: string, props?: cdk.StackProps) {
        super(scope, id, props);

        // ================== DynamoDB Tables ==================
        const conversationsTable = new dynamodb.Table(this, 'ConversationsTable', {
            tableName: 'ChatbotInteligenteConversations',
        ...
        const knowledgeBaseTable = new dynamodb.Table(this, 'KnowledgeBaseTable', {
            tableName: 'ChatbotInteligenteKnowledgeBase',
            partitionKey: { name: 'PK', type: dynamodb.AttributeType.STRING },
            sortKey: { name: 'SK', type: dynamodb.AttributeType.STRING },
            billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
            removalPolicy: cdk.RemovalPolicy.DESTROY,
        ...
        const analyticsTable = new dynamodb.Table(this, 'AnalyticsTable', {
            tableName: 'ChatbotInteligenteAnalytics',
            partitionKey: { name: 'PK', type: dynamodb.AttributeType.STRING },
            sortKey: { name: 'SK', type: dynamodb.AttributeType.STRING },
            billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
            removalPolicy: cdk.RemovalPolicy.DESTROY,
        ...
        const dataBucket = new s3.Bucket(this, 'DataBucket', {
            bucketName: `chatbot-inteligente-data-${this.account}-${this.region}`,
            removalPolicy: cdk.RemovalPolicy.DESTROY,
        ...
        const orchestratorFunction = new lambda.Function(this, 'OrchestratorFunction', {
            functionName: 'ChatbotInteligenteOrchestrator',
            ...
            environment: {
                ...
                LEX_BOT_ID: 'X2LPJ7ULSY',
                LEX_BOT_ALIAS_ID: 'TSTALIASID', // Necesito crear el alias o usar uno default. El script create_bot crea DRAFT. Necesitare un alias.
                LOG_LEVEL: 'INFO',
            },
        });

        const fulfillmentFunction = new lambda.Function(this, 'FulfillmentFunction', {
            functionName: 'ChatbotInteligenteFulfillment',
            runtime: lambda.Runtime.PYTHON_3_11,
            handler: 'handler.lambda_handler',
        ...
        const frontendBucket = new s3.Bucket(this, 'FrontendBucket', {
            bucketName: `chatbot-inteligente-frontend-${this.account}-${this.region}`,
            removalPolicy: cdk.RemovalPolicy.DESTROY,
            autoDeleteObjects: true,
            blockPublicAccess: s3.BlockPublicAccess.BLOCK_ALL,
        });
...
// Crear el stack
// Crear el stack
new ChatbotInteligenteStack(app, 'ChatbotInteligenteStack', {
    env,
    description: 'Chatbot Inteligente TostiCafe',
});
